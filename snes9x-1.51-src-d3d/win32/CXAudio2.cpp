...
extern void S9xMixSamplesNoLimit(uint8 *buffer, int sample_count);
...
bool CXAudio2::SetupSound(uint8 **syncSoundBuffer,int *sample_count)
{
	...
	*sample_count = 0;
	...
	buffer_sampleCount = (Settings.SoundPlaybackRate * mixInterval * (Settings.Stereo ? 2 : 1)) / 1000;
	...
	for(int i=0;i<bufferCount;i++) {
		...
		S9xMixSamplesNoLimit(bufferBase,buffer_sampleCount);
	...
	while(emptyBuffers) {
		...
		if(Settings.SoundSync) {
			...
			if(so.samples_mixed_so_far) {
				...
				XMemCpy(curBuffer,syncSoundBuffer,mixed_bytes);
			}
			S9xMixSamplesNoLimit(curBuffer+mixed_bytes,buffer_sampleCount-so.samples_mixed_so_far);
			so.samples_mixed_so_far = 0;
			LeaveCriticalSection(&GUI.SoundCritSect);
		}
		else
			S9xMixSamplesNoLimit(curBuffer,buffer_sampleCount);
