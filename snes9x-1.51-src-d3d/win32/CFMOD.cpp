...
to hear any sound effects or music while playing.",
	...
	syncSoundBuffer = new bool8[bufferSize];
	return true;
}
...
F_CALLBACKAPI CFMOD::FMODStreamCallback (FSOUND_STREAM *stream, void *buff, int len, void *param)
{
	...
	if(Settings.SoundSync) {
		...
		S9xMixSamplesNoLimit((unsigned char *)buff+mixed_bytes,sample_count-so.samples_mixed_so_far);
		so.samples_mixed_so_far = 0;
		LeaveCriticalSection(&GUI.SoundCritSect);
	}
	else
		S9xMixSamplesNoLimit ((unsigned char *) buff, sample_count);
