...
inline void DoubleLine16( uint16 *  __restrict lpDst, uint16 *lpSrc, unsigned int Width){
    while(Width--){
		*lpDst++ = *lpSrc;
		*lpDst++ = *lpSrc++;
    }
}
inline void TripleLine16( uint16 *  __restrict lpDst, uint16 *lpSrc, unsigned int Width){
    while(Width--){
		*lpDst++ = *lpSrc;
		*lpDst++ = *lpSrc;
		*lpDst++ = *lpSrc++;
...
inline void ThreeHalfLine16( uint16 *  __restrict lpDst, uint16 *lpSrc, unsigned int Width){
    while(Width-=2){
		*lpDst++ = *lpSrc;
...
inline void HalfLine16( uint16 *  __restrict lpDst, uint16 *lpSrc, unsigned int Width){
    while(Width--){
		*lpDst++ = Interp05(*lpSrc, *(lpSrc+1));
		lpSrc+=2;
    }
}
inline void DoubleLine32( uint32 *  __restrict lpDst, uint16 *lpSrc, unsigned int Width){
    while(Width--){
...
inline void SingleLine32( uint32 *   __restrict lpDst, uint16 *lpSrc, unsigned int Width){
	while(Width--){
		*lpDst++ = CONVERT_16_TO_32(*lpSrc);
		lpSrc++;
	}
}
inline void HalfLine32( uint32 *  __restrict lpDst, uint16 *lpSrc, unsigned int Width){
    while(Width--){
...
inline void TripleLine32( uint32 *  __restrict lpDst, uint16 *lpSrc, unsigned int Width){
    while(Width--){
		*lpDst++ = CONVERT_16_TO_32(*lpSrc);
		*lpDst++ = CONVERT_16_TO_32(*lpSrc);
		*lpDst++ = CONVERT_16_TO_32(*lpSrc);
...
inline void ThreeHalfLine32( uint32 *  __restrict lpDst, uint16 *lpSrc, unsigned int Width){
    while(Width-=2){
		*lpDst++ = CONVERT_16_TO_32(*lpSrc);
		const uint16 color = Interp05(*lpSrc, *(lpSrc+1));
		*lpDst++ = CONVERT_16_TO_32(color);
...
void RenderPlain (SSurface Src, SSurface Dst, RECT *rect)
{
	...
	uint16 * __restrict  lpSrc = reinterpret_cast<uint16 *>(Src.Surface);
	const unsigned int srcPitch = Src.Pitch >> 1;

	if(GUI.ScreenDepth == 16)
	{
		const unsigned int dstPitch = Dst.Pitch >> 1;
		uint16 * __restrict  lpDst = reinterpret_cast<uint16 *>(Dst.Surface) + rect->top * dstPitch + rect->left;

		for (unsigned int H = 0; H != srcHeight; H++, lpDst += dstPitch, lpSrc += srcPitch)
			XMemCpy (lpDst, lpSrc, Src.Width << 1);
	}
	else if(GUI.ScreenDepth == 32)
	{
		const unsigned int dstPitch = Dst.Pitch >> 2;
		uint32 * __restrict  lpDst = reinterpret_cast<uint32 *>(Dst.Surface) + rect->top * dstPitch + rect->left;
		...
		if(Src.Height <= SNES_HEIGHT_EXTENDED)
			if(Src.Width != 512)
				for (H = 0; H < srcHeight; H++, lpSrc += srcPitch)
					...
					XMemSet (lpDst, 0, 512*2), lpDst += dstPitch;
			else
				for (H = 0; H < srcHeight; H++, lpSrc += srcPitch)
					XMemCpy (lpDst, lpSrc, Src.Width << 1), lpDst += dstPitch,
					XMemSet (lpDst, 0, 512*2), lpDst += dstPitch;
		else
			if(Src.Width != 512)
		...
		if(Src.Height <= SNES_HEIGHT_EXTENDED)
			if(Src.Width != 512)
				for (H = 0; H < srcHeight; H++, lpSrc += srcPitch)
					...
					XMemSet (lpDst, 0, 512*4), lpDst += dstPitch;
			else
				for (H = 0; H < srcHeight; H++, lpSrc += srcPitch)
...
void RenderSuperEagle (SSurface Src, SSurface Dst, RECT *rect)
{
    ...
    unsigned char * __restrict lpSrc, * __restrict lpDst;
...
static inline int Diff(int c1, int c2)
{
...
void __inline RenderHQ2X (SSurface Src, SSurface  Dst, RECT *rect)
{
    ...
    uint8 * __restrict srcPtr = Src.Surface;
    uint8 * __restrict dstPtr = Dst.Surface;
    uint32 srcPitch = Src.Pitch;
    uint32 dstPitch = Dst.Pitch;
    int width = Src.Width;
    int height = Src.Height;
	...
	uint16	* __restrict sp = (uint16 *) srcPtr;
	uint16	*  __restrict dp = (uint16 *) dstPtr;

	const int * __restrict  RGBtoYUVtable = RGBtoYUV;
