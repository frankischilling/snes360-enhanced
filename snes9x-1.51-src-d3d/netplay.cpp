...
bool8 S9xNPConnect ()
{
	...
	{
	    XMemCpy ((char *)&address.sin_addr, hostinfo->h_addr,
		    hostinfo->h_length);
	}
	else
	{
...
up correctly?");
    ...
    {
	XMemCpy ((char *)&address.sin_addr, &addr, sizeof (addr));
    }
...
void S9xNPResetJoypadReadPos ()
{
    ...
    for (int h = 0; h < NP_JOYPAD_HIST_SIZE; h++)
        XMemSet ((void *) &NetPlay.Joypads [h], 0, sizeof (NetPlay.Joypads [0]));
    for (int h = 0; h < NP_JOYPAD_HIST_SIZE; h++)
        XMemSet ((void *) &NetPlay.JoypadsReady [h], 0, sizeof (NetPlay.JoypadsReady [0]));
}
