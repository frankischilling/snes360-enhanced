...
void S9xResetBSX(void)
{
	if (Settings.BSXItself)
		XMemSet(Memory.ROM, 0, FLASH_SIZE);

	XMemSet(BSX.PPU, 0, sizeof(BSX.PPU));
	XMemSet(BSX.MMC, 0, sizeof(BSX.MMC));
	XMemSet(BSX.prevMMC, 0, sizeof(BSX.prevMMC));
	...
	BSX.out_index = 0;
	XMemSet(BSX.output, 0, sizeof(BSX.output));
