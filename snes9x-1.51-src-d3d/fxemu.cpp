...
void fx_setCache()
{
	...
	t1 = &ROM(c);
	t2 = &ROM(0);
	XMemCpy(GSU.pvCache,t1,i);
	XMemCpy(&GSU.pvCache[i],t2,512-i);
    }
}
#endif
...
static void fx_backupCache()
{
	...
	for(i=0; i<32; i++)
	{
		...
		if(c < (0x10000-16))
		{
		    ...
		    XMemCpy(&GSU.avCacheBackup[i<<4],t,16);
		    XMemCpy(t,&GSU.pvCache[i<<4],16);
		}
		else
		{
		    uint8 * t1;
		    ...
		    t1 = &GSU.pvPrgBank[c];
		    t2 = &GSU.pvPrgBank[0];
		    XMemCpy(&GSU.avCacheBackup[i<<4],t1,a);
		    XMemCpy(t1,&GSU.pvCache[i<<4],a);
		    XMemCpy(&GSU.avCacheBackup[(i<<4)+a],t2,16-a);
		    XMemCpy(t2,&GSU.pvCache[(i<<4)+a],16-a);
		}
	    }
	    c = USEX16(c+16);
	    v >>= 1;
...
static void fx_restoreCache()
{
	...
	for(i=0; i<32; i++)
	{
		...
		{
		    uint8 * t = &GSU.pvPrgBank[c];
		    XMemCpy(t,&GSU.avCacheBackup[i<<4],16);
		    XMemCpy(&GSU.pvCache[i<<4],t,16);
		}
		else
		{
		    uint8 * t1;
		    ...
		    t1 = &GSU.pvPrgBank[c];
		    t2 = &GSU.pvPrgBank[0];
		    XMemCpy(t1,&GSU.avCacheBackup[i<<4],a);
		    XMemCpy(&GSU.pvCache[i<<4],t1,a);
		    XMemCpy(t2,&GSU.avCacheBackup[(i<<4)+a],16-a);
		    XMemCpy(&GSU.pvCache[(i<<4)+a],t2,16-a);
		}
	    }
	    c = USEX16(c+16);
	    v >>= 1;
...
void FxReset(struct FxInit_s *psFxInfo)
{
    ...
    XMemSet((uint8*)&GSU,0,sizeof(struct FxRegs_s));
    ...
    XMemSet(GSU.pvRegisters,0,0x300);
