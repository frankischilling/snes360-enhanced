...
static void S9xDeinterleaveType1 (int size, uint8 *base)
{
	...
	uint8 * __restrict tmp = (uint8 *) XMemAlloc(0x8000, g_dwPhysicalMemoryAttributes);
	if (tmp)
	{
		...
		XMemFree(tmp, g_dwPhysicalMemoryAttributes);
	}
}
...
{
    RAM	 = (uint8 *) XMemAlloc(0x20000, g_dwPhysicalMemoryAttributes);
	...
	XMemSet(RAM,  0x00, 0x20000);
	XMemSet(SRAM, 0x00, 0x20000);
	XMemSet(VRAM, 0x00, 0x10000);
	XMemSet(ROM,  0x00, MAX_ROM_SIZE + 0x200 + 0x8000);

	XMemSet(IPPU.TileCache[TILE_2BIT],       0x00, MAX_2BIT_TILES * 64);
	XMemSet(IPPU.TileCache[TILE_4BIT],       0x00, MAX_4BIT_TILES * 64);
	XMemSet(IPPU.TileCache[TILE_8BIT],       0x00, MAX_8BIT_TILES * 64);
	XMemSet(IPPU.TileCache[TILE_2BIT_EVEN],  0x00, MAX_2BIT_TILES * 64);
	XMemSet(IPPU.TileCache[TILE_2BIT_ODD],   0x00, MAX_2BIT_TILES * 64);
	XMemSet(IPPU.TileCache[TILE_4BIT_EVEN],  0x00, MAX_4BIT_TILES * 64);
	XMemSet(IPPU.TileCache[TILE_4BIT_ODD],   0x00, MAX_4BIT_TILES * 64);

	XMemSet(IPPU.TileCached[TILE_2BIT],      0x00, MAX_2BIT_TILES);
	XMemSet(IPPU.TileCached[TILE_4BIT],      0x00, MAX_4BIT_TILES);
	XMemSet(IPPU.TileCached[TILE_8BIT],      0x00, MAX_8BIT_TILES);
	XMemSet(IPPU.TileCached[TILE_2BIT_EVEN], 0x00, MAX_2BIT_TILES);
	XMemSet(IPPU.TileCached[TILE_2BIT_ODD],  0x00, MAX_2BIT_TILES);
	XMemSet(IPPU.TileCached[TILE_4BIT_EVEN], 0x00, MAX_4BIT_TILES);
	XMemSet(IPPU.TileCached[TILE_4BIT_ODD],  0x00, MAX_4BIT_TILES);
...
void CMemory::Deinit (void)
{
	...
	if (RAM)
	{
		XMemFree(RAM, g_dwPhysicalMemoryAttributes);
		RAM = NULL;
	}
...
uint32 CMemory::FileLoader (uint8 *buffer, const char *filename, int32 maxsize)
{
	...
	XMemSet(NSRTHeader, 0, sizeof(NSRTHeader));
	HeaderCount = 0;
...
bool8 CMemory::LoadROM (const char *filename)
{
	...
	XMemSet(ROM, 0x00, MAX_ROM_SIZE);
	XMemSet(&Multi, 0x00, sizeof(Multi));
...
again:
	...
	XMemSet(&SNESGameFixes, 0x00, sizeof(SNESGameFixes));
	...
	if (strcmp(strlwr(ROMName),"super metroid") == 0)
	{
		//DoAchievo(2);  // Achievements disabled
	}
	else if (strcmp(strlwr(ROMName), "super punch-out!!") == 0)
	{
		//DoAchievo(4);  // Achievements disabled
	}
	else if (strcmp(strlwr(ROMName), "super mario kart") == 0)
	{
		//DoAchievo(3);  // Achievements disabled
	}
	else if (strcmp(strlwr(ROMName),"super marioworld") == 0)
	{
		//DoAchievo(6);  // Achievements disabled
	}
	else if (strcmp(strlwr(ROMName),"star fox 2") == 0)
	{
		//DoAchievo(ACHIEVEMENT_LEAKER);  // Achievements disabled
	}
...
void CMemory::ClearSRAM (bool8 onlyNonSavedSRAM)
{
	...
	XMemSet(SRAM, SNESGameFixes.SRAMInitialValue, 0x20000);
}
...
bool8 CMemory::LoadSRAM (const char *filename)
{
	...
	if (Multi.cartType && Multi.sramSizeB)
	{
		...
		contentData.DeviceID = snesStoreage.GetSignedInUser();
		contentData.dwContentType = XCONTENTTYPE_SAVEDGAME;
		swprintf_s(contentData.szDisplayName,L"Snes360Save");
		strcpy_s(contentData.szFileName,S9xGetFilename(".srm", SRAM_DIR));

		snesStoreage.ReadSaveGame(&contentData, (char *)Multi.sramB, 0x10000, (DWORD *)len);
	...
	if (size)
	{
		contentData.DeviceID = snesStoreage.m_DeviceID;
		contentData.dwContentType = XCONTENTTYPE_SAVEDGAME;
		swprintf_s(contentData.szDisplayName,L"Snes360Save");
		strcpy_s(contentData.szFileName,S9xGetFilename(".srm", SRAM_DIR));

		snesStoreage.ReadSaveGame(&contentData, (char *)SRAM, 0x20000, (DWORD *)len);
...
bool8 CMemory::SaveSRAM (const char *filename)
{
	...
	wchar_t romName[50];
	swprintf_s(romName, L"%S", Memory.ROMName);
...
void CMemory::ParseSNESHeader (uint8 *RomHeader)
{
	...
	if (bs)
		XMemSet(ROMName + 16, 0x20, ROM_NAME_LEN - 17);
...
{
	XMemSet(MemorySpeed, SLOW_ONE_CYCLE, 0x1000);
