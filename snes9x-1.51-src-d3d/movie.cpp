...
static void read_frame_controller_data(bool addFrame)
{
	...
	for(int port=0;port<2;port++)
	{
		if(Movie.PortType[port] == CTL_MOUSE)
		{
			...
			XMemCpy(buf, Movie.InputBufferPtr, MOUSE_DATA_SIZE); Movie.InputBufferPtr += MOUSE_DATA_SIZE;
			MovieSetMouse(port, buf, !addFrame);
		}
		if(Movie.PortType[port] == CTL_SUPERSCOPE)
		{
			uint8 buf [SCOPE_DATA_SIZE];
			XMemCpy(buf, Movie.InputBufferPtr, SCOPE_DATA_SIZE); Movie.InputBufferPtr += SCOPE_DATA_SIZE;
			MovieSetScope(port, buf);
		}
		if(Movie.PortType[port] == CTL_JUSTIFIER)
		{
			uint8 buf [JUSTIFIER_DATA_SIZE];
			XMemCpy(buf, Movie.InputBufferPtr, JUSTIFIER_DATA_SIZE); Movie.InputBufferPtr += JUSTIFIER_DATA_SIZE;
...
static void write_frame_controller_data()
{
	...
	for(int port=0;port<2;port++)
	{
		if(Movie.PortType[port] == CTL_MOUSE)
		{
			...
			XMemCpy(Movie.InputBufferPtr, buf, MOUSE_DATA_SIZE); Movie.InputBufferPtr += MOUSE_DATA_SIZE;
		}
		if(Movie.PortType[port] == CTL_SUPERSCOPE)
		{
			uint8 buf [SCOPE_DATA_SIZE];
			MovieGetScope(port, buf);
			XMemCpy(Movie.InputBufferPtr, buf, SCOPE_DATA_SIZE); Movie.InputBufferPtr += SCOPE_DATA_SIZE;
		}
		if(Movie.PortType[port] == CTL_JUSTIFIER)
		{
			uint8 buf [JUSTIFIER_DATA_SIZE];
			MovieGetJustifier(port, buf);
			XMemCpy(Movie.InputBufferPtr, buf, JUSTIFIER_DATA_SIZE); Movie.InputBufferPtr += JUSTIFIER_DATA_SIZE;
		}
	}
#endif
}
...
static void S9xMovieResetControls ()
{
	...
	XMemSet(clearedMouse, 0, MOUSE_DATA_SIZE);
	clearedMouse[4] = 1; // mouse[n].buttons = 1;

	uint8 clearedScope [SCOPE_DATA_SIZE];
	XMemSet(clearedScope, 0, SCOPE_DATA_SIZE);

	uint8 clearedJustifier [JUSTIFIER_DATA_SIZE];
	XMemSet(clearedJustifier, 0, JUSTIFIER_DATA_SIZE);
...
void S9xMovieUpdateOnReset ()
{
	switch(Movie.State)
	{
		...
		{
			reserve_buffer_space((uint32)((Movie.InputBufferPtr+Movie.BytesPerSample)-Movie.InputBuffer));
			XMemSet(Movie.InputBufferPtr, 0xFF, Movie.BytesPerSample);
			Movie.InputBufferPtr += Movie.BytesPerSample;
...
void S9xMovieFreeze (uint8** buf, uint32* size)
{
	...
	*size=size_needed;
	...
	XMemCpy(ptr, Movie.InputBuffer, Movie.BytesPerSample * (Movie.MaxSample+1));
}
...
//		S9xMessage(S9X_INFO, S9X_MOVIE_INFO, MOVIE_INFO_RERECORD);
		...
		XMemCpy(Movie.InputBuffer, ptr, space_needed);
		flush_movie();
		fseek(Movie.File, Movie.ControllerDataOffset+(Movie.BytesPerSample * (Movie.CurrentSample+1)), SEEK_SET);
	}
	else
