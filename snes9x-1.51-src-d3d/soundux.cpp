...
void S9xSetEchoEnable (uint8 byte)
{
	...
	{
		XMemSet (Loop, 0, sizeof (Loop));
		XMemSet (Echo, 0, sizeof (Echo));
	}
#endif
...
static void AltDecodeBlock (Channel *ch)
{
	if (ch->block_pointer > 0x10000 - 9)
	{
		...
		ch->loop = FALSE;
		ch->block = ch->decoded;
		XMemSet ((void *) ch->decoded, 0, sizeof (int16) * 16);
		return;
	}
...
void S9xMixSamples (uint8 *buffer, int sample_count)
{
	...
	if (!so.mute_sound)
	{
		XMemSet (MixBuffer, 0, sample_count * sizeof (MixBuffer[0]));
		if (!Settings.DisableSoundEcho)
			XMemSet (EchoBuffer, 0, sample_count * sizeof (EchoBuffer[0]));
	...
	if (so.sixteen_bit)
	{
		// 16-bit sound
		if (so.mute_sound)
			XMemSet (buffer, 0, sample_count << 1);
		else
		{
			if (!Settings.DisableSoundEcho)
			{
    ...
    else
    {
		// 8-bit sound
		if (so.mute_sound)
			XMemSet (buffer, 128, sample_count);
		else
	#ifdef __sun
		if (so.encoded)
		{
