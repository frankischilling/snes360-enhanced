...
s9xcommand_t S9xGetCommandT(const char *name){
    ...
    if(!strcmp(name,"None")){
        ...
        if(c==NULL){
            perror("XMemAlloc error while parsing multi");
            return cmd;
        }
        n=0; i=(name[0]=='+')?2:1;
        do {
...
#define COPY(x) {XMemCpy((char*)s->internal+i, &(x), sizeof(x)); i+=sizeof(x);}
	i=0;
	for(j=0; j<8; j++)
...
void S9xControlPostLoad(struct SControlSnapshot *s){
	...
	if(s->ver>1)
	{
#define COPY(x) {XMemCpy(&(x), (char*)s->internal+i, sizeof(x)); i+=sizeof(x);}
		i=0;
		for(j=0; j<8; j++)
			COPY(joypad[j].buttons);
		for(j=0; j<2; j++) {
