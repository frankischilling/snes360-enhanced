...
void S9xSoftResetPPU ()
{
	...
	for (c = 0; c < 2; c++)
		XMemSet (&IPPU.Clip [c], 0, sizeof (struct ClipData));

	for (c = 0; c < 0x8000; c += 0x100)
		XMemSet (&Memory.FillRAM [c], c >> 8, 0x100);
...
void S9xSuperFXExec ()
{
	...
	*(uint16 *) &SRAM [0x002c] = 0x8000;
	...
	while (Memory.FillRAM [0x3030] & 0x20)
	{
	    ...
	    if(strlen(tmp) < 32) { XMemSet(&tmp[strlen(tmp)],' ',32-strlen(tmp)); tmp[32] = 0; }

	    /* Copy registers (so we can see if any changed) */
	    vColr = FxGetColorRegister();
	    vPor = FxGetPlotOptionRegister();
	    XMemCpy(avReg,SuperFX.pvRegisters,0x40);
