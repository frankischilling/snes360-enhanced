...
struct SGFX{
    ...
    uint16 * __restrict Screen;
    uint16 * __restrict SubScreen;
    uint8  * __restrict ZBuffer;
    uint8  * __restrict SubZBuffer;
    uint32 Pitch;
    uint32 ScreenSize;
    ...
    uint16 * __restrict X2;
    uint16 * __restrict ZERO_OR_X2;
    uint16 * __restrict ZERO;

    uint32 RealPPL;      // True PPL of Screen buffer.
    uint32 PPL;	         // Number of pixels on each of Screen buffer
    uint32 LinesPerTile; // number of lines in 1 tile (4 or 8 due to interlace)
    uint16 * __restrict ScreenColors;     // Screen colors for rendering main
