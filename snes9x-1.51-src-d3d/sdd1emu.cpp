...
void SDD1_decompress(uint8 *out, uint8 *in, int len){
    ...
    valid_bits=5;
    in_buf=in+2;
    XMemSet(bit_ctr, 0, sizeof(bit_ctr));
    XMemSet(context_states, 0, sizeof(context_states));
    XMemSet(context_MPS, 0, sizeof(context_MPS));
    XMemSet(prev_bits, 0, sizeof(prev_bits));
