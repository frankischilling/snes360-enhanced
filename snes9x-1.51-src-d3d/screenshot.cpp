...
#ifdef HAVE_LIBPNG
#ifdef _XBOX
// Custom I/O functions for png_set_write_fn to work with HANDLE
static void png_write_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
    HANDLE fp = (HANDLE)png_get_io_ptr(png_ptr);
    DWORD written;
    WriteFile(fp, data, (DWORD)length, &written, NULL);
}
...
bool8 S9xDoScreenshot(int width, int height)
{
	...
	HANDLE fp = CreateFile( fname, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS , 0, NULL );
...
bool8 S9xDoScreenshot(int width, int height, char *fname)
{
#ifdef HAVE_LIBPNG
    //FILE *fp;
    png_structp png_ptr;
	...
	HANDLE fp = CreateFile( fname, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS , 0, NULL );
