...
bool8 S9xInitAPU ()
{
    IAPU.RAM = (uint8 *) XMemAlloc (0x10000,g_dwPhysicalMemoryAttributes);
...
void S9xDeinitAPU ()
{
    if (IAPU.RAM)
    {
		XMemFree ((char *) IAPU.RAM, g_dwPhysicalMemoryAttributes);
		IAPU.RAM = NULL;
    }
}
...
void S9xResetAPU ()
{
	...
	XMemSet(spc_dump_dsp, 0x00,0x100);
	XMemSet(IAPU.RAM, 0x00, 0x100);
	XMemSet(IAPU.RAM+0x20, 0xFF, 0x20);
	XMemSet(IAPU.RAM+0x60, 0xFF, 0x20);
	XMemSet(IAPU.RAM+0xA0, 0xFF, 0x20);
	XMemSet(IAPU.RAM+0xE0, 0xFF, 0x20);

	for(i=1;i<256;i++)
	{
		XMemCpy(IAPU.RAM+(i<<8), IAPU.RAM, 0x100);
	}

    XMemSet (APU.OutPorts, 0x00, 4);
