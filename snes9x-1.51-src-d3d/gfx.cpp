...
bool8 S9xGraphicsInit(){
    S9xInitTileRenderer();
    XMemSet(BlackColourMap, 0x00, 256*sizeof(uint16));
    ...
    XMemSet(GFX.ZERO, 0x00, 0x10000*sizeof(uint16));
    for(r=0; r<=MAX_RED; r++){
...
void S9xStartScreenRefresh(){
    ...
    if(IPPU.RenderThisFrame){
        ...
        XMemSet(GFX.ZBuffer, 0x00, GFX.ScreenSize);
        XMemSet(GFX.SubZBuffer, 0x00, GFX.ScreenSize);
    }
...
void S9xSetupOBJ() {
    ...
    if(!PPU.OAMPriorityRotation || !(PPU.OAMFlip&PPU.OAMAddr&1)){
        ...
        XMemSet(LineOBJ, 0, sizeof(LineOBJ));
        ...
        XMemSet(OBJOnLine, 0, sizeof(OBJOnLine));
